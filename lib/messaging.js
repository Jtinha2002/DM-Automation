...
// Destinatário: comentar não abre a janela normal de mensagens. A 1ª mensagem
// em resposta a um comentário precisa usar "resposta privada" (recipient.comment_id);
// só depois que a pessoa interage (ex: toca num botão) é que vale recipient.id normal.
function buildRecipient(userId, commentId) {
  return commentId ? { comment_id: commentId } : { id: userId };
}

// ── Send a list of blocks; records each into the conversation ──
async function sendBlocks(account, userId, username, blocks, vars = {}, { source = 'auto', commentId = null } = {}) {
  const conv = getOrCreateConversation(account.id, userId, username);
  const results = [];

  // A Meta só aceita comment_id como destinatário na 1ª mensagem enviada por
  // comentário; as próximas (se houver mais blocos) precisam do id normal.
  let firstSend = true;
  for (const block of blocks) {
    ...
    try {
      const res = await api.post(`${GRAPH}/${account.instagram_user_id}/messages`, {
        recipient: buildRecipient(userId, firstSend ? commentId : null),
        message: msgObj,
...
async function sendQuickReply(account, userId, username, text, replies, { source = 'auto', commentId = null } = {}) {
  const conv = getOrCreateConversation(account.id, userId, username);
  const res = await api.post(`${GRAPH}/${account.instagram_user_id}/messages`, {
    recipient: buildRecipient(userId, commentId),
